"""
load_quaternary_faults.py
Unworked Gold — USGS Quaternary Fault and Fold Database loader.

Scope: Layer 21 (National Database) — all ~112,809 faults, all states.
State-specific layers 0-20 skipped (subsets of National Database).

Reads credentials from .env per F.9.

Run (safe to re-run — resumes, skips persistent failures):
    python load_quaternary_faults.py
"""
...
# ── Config ─────────────────────────────────────────────
BASE = "https://earthquake.usgs.gov/arcgis/rest/services/haz/Qfaults/MapServer"
LAYER_ID = 21
TABLE = "quaternary_faults"
PAGE_SIZE = 1000
HTTP_TIMEOUT = 60
HTTP_RETRIES = 3
HTTP_BACKOFF = 3
...
# ── Main ingest loop ───────────────────────────────────
print(f"\n=== Quaternary Faults (layer {LAYER_ID} → {TABLE}) ===")
offset = existing
total = 0
page_num = 0
failed_offsets = []
